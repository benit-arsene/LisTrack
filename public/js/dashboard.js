...
function updateFirstSiteVisitedLine(firstVisitDomain, firstVisitIso) {
  ...
  el.textContent = `First site visited · ${firstVisitDomain}`;
  el.classList.remove("hidden");
}
...
function getDisplayName(domain) {
  ...
  const subdomainParts = parts
    .slice(0, parts.length - 2)
    .filter((p) => p !== "www");
  if (subdomainParts.length > 0) {
    const subdomainStr = subdomainParts
...
function renderDailyChart(data) {
  ...
  breakdown.forEach((item, i) => {
    ...
    if (ratio > 0.8)
      ...
      color = isDark ? "rgba(245,158,11,0.7)" : "rgba(245,158,11,0.6)";
    else if (ratio > 0.2)
      color = isDark ? "rgba(16,185,129,0.7)" : "rgba(16,185,129,0.6)";
    else color = barColor;
    ...
    const labelWide = safeFormatDate(item.date, {
      month: "short",
      day: "numeric",
    }); // "Jul 11"
  ...
  // Average
  const avg =
    breakdown.reduce((s, d) => s + d.totalMinutes, 0) / breakdown.length;
...
function offsetPeriod(dateStr, delta) {
  ...
  else if (currentPeriod === "7days" || currentPeriod === "week")
...
function formatPeriodLabel(data) {
  if (currentPeriod === "30days") {
    const s = safeFormatDate(data.startDate, {
      month: "short",
      day: "numeric",
    });
    const e = safeFormatDate(data.endDate, {
      month: "short",
      day: "numeric",
      year: "numeric",
    });
...
async function fetchData({ silent = false } = {}) {
  ...
  try {
    ...
    if (currentPeriod !== "day" && currentPeriod !== "custom") {
      trendsPromise = fetch(
        await apiUrl("/trends", { period: currentPeriod, date: currentDate }),
      )
...
function createDomainRow(item, index) {
  ...
  row.className =
    "group rank-item px-6 py-4 flex items-center gap-4 cursor-pointer dark:hover:bg-gray-700/30";
...
function renderDomainList(data, container) {
  ...
  visibleDomains.forEach((item, index) => {
    const percentage =
      maxMinutes > 0 ? (item.totalMinutes / maxMinutes) * 100 : 0;
  ...
  if (showMoreContainer) {
    if (visibleDomainCount < data.domains.length) {
      ...
      document.getElementById("showMoreText").textContent =
        `Show ${Math.min(remaining, 10)} more (${visibleDomainCount + Math.min(remaining, 10)} of ${data.domains.length})`;
    } else {
...
function openDomainModal(domain) {
  ...
  if (faviconLink) {
    faviconLink.href = domainUrl;
    faviconLink.title = "Open " + domain;
  }
  if (titleLink) {
    titleLink.href = domainUrl;
    titleLink.title = "Open " + domain;
  }
  document.getElementById("modalBreakdown").innerHTML =
    '<p class="text-sm text-gray-400 dark:text-gray-500 text-center py-4">Loading breakdown...</p>';
...
async function loadDomainBreakdown(domain) {
  try {
    ...
    document.getElementById("modalTotal").textContent =
      `${formatTime(totalMin)} total · ${totalVisits} visits`;

    if (breakdown.length === 0) {
      document.getElementById("modalBreakdown").innerHTML =
        '<p class="text-sm text-gray-400 dark:text-gray-500 text-center py-4">No detailed data available.</p>';
      return;
    }
    ...
    breakdown.slice(0, 14).forEach((item) => {
      ...
      const label = safeFormatDate(item.date, {
        weekday: "short",
        month: "short",
        day: "numeric",
      });
    ...
    document.getElementById("modalBreakdown").innerHTML =
      '<p class="text-sm text-red-400 dark:text-red-400 text-center py-4">Failed to load breakdown.</p>';
  }
}
...
async function fetchGoals({ silent = false } = {}) {
  ...
  try {
    ...
    if (statusResp.ok) {
      ...
      (statusData.goals || []).forEach((s) => {
        statusMap[s.domain] = s;
      });
    }
    ...
    if (el) {
      el.classList.remove("hidden");
      el.querySelector("p").textContent = "Could not load goals.";
    }
  }
}
...
function updateGoalsSummary(goals) {
  ...
  if (exceeded > 0)
    parts.push(
      `<span class="text-red-500 dark:text-red-400 font-semibold">${exceeded} exceeded</span>`,
    );
  if (approaching > 0)
    parts.push(
      `<span class="text-amber-500 dark:text-amber-400 font-semibold">${approaching} approaching</span>`,
    );
...
function getGoalStatusBadge(goal) {
  if (!goal.enabled)
    return `<span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-gray-100 dark:bg-gray-700 text-gray-400 dark:text-gray-500">Disabled</span>`;
  if (goal.exceeded)
    return `<span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400">Exceeded</span>`;
  if (goal.approaching)
    return `<span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400">${goal.percentage}%</span>`;
  if (goal.percentage > 0)
    return `<span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400">${goal.percentage}%</span>`;
...
function renderGoals(goals) {
  ...
  if (goals.length === 0) {
    ...
    goalsEmpty.querySelector("p").textContent =
...
async function toggleGoal(id, enabled) {
  try {
    ...
    console.error("[goals] Error toggling goal:", err);
  }
}
...
function updateTrackedDomainsList(data) {
  ...
  datalist.innerHTML = data.domains
    .map((d) => `<option value="${d.domain}">`)
    .join("");
}
...
function quickAddGoal(domain) {
  ...
  if (goalsSection)
    goalsSection.scrollIntoView({ behavior: "smooth", block: "center" });
  setTimeout(() => {
    minutesInput?.focus();
    minutesInput?.select();
  }, 400);
...
function silentUpdate(data) {
  ...
  const incomingDomains = data.domains
    .slice(0, visibleDomainCount)
    .map((d) => d.domain);
    ...
    if (
      ...
      if (pct === 0) {
        ...
        trendBadge.innerHTML =
          (isUp ? "▲" : "▼") + " " + Math.abs(pct) + "% vs last period";
      }
    } else {
...
function setTheme(isDark) {
  ...
  try {
    localStorage.setItem("lisTrackTheme", isDark ? "dark" : "light");
  } catch (_) {}
...
function initTheme() {
  const saved = (() => {
    try {
      return localStorage.getItem("lisTrackTheme");
    } catch (_) {
      return null;
    }
  })();
...
function exportCSV() {
  ...
  // Build CSV rows
  const rows = [["Domain", "Total Minutes", "Formatted Time"]];
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 403)
        alert(
          "Seed data is only available when running locally (SQLite).\nIt does not work on the production server.",
        );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (
    urlPeriod &&
    ["day", "week", "month", "7days", "30days", "custom"].includes(urlPeriod)
  )
